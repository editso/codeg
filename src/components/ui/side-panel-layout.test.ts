...
describe("transcript inside a side panel", () => {
  ...
  it("does not re-pad a transcript the virtualizer already insets", () => {
    ...
    // The fork intentionally uses its wider 920px conversation rail. Drawers
    // must inherit that same single gutter rather than reverting the upstream
    // 3xl cap or adding a second wrapper padding.
    expect(virtualized).toContain('"mx-auto max-w-[920px] px-4"')
