...
// The fork retains a Quick actions row next to Search. Its dialogs and their
// workspace providers are covered by quick-actions-dropdown.test.tsx; expose
// only the trigger here so this sidebar suite remains scoped to navigation and
// view options.
vi.mock("./quick-actions-dropdown", () => ({
  QuickActionsDropdown: ({ trigger }: { trigger: ReactNode }) => trigger,
}))
...
describe("Sidebar — fixed nav region", () => {
  ...
  it("keeps the fork's Search row and shortcut in the expanded sidebar", () => {
    const { getByText } = renderSidebar()
    // Local product behavior deliberately retains this row even though the
    // upstream window chrome also exposes search.
